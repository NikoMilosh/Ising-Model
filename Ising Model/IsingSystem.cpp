...
IsingSystem::IsingSystem(Window *set_win) {
	...
	logfile.open("magnetisation.txt");
	//logfile.open("energy.txt");
...
double IsingSystem::MagnetCalc() {
	...
	double Magnetisation=(1/N)*(blue+green);

	// if(logfile.is_open()){logfile << Magnetisation << endl;}
	//if(green %1.0 == 0.0 || green % 100.0 == 0.0 && logfile.is_open()){logfile << Magnetisation << endl;}
...
double IsingSystem::energy() {
	...
	for (int i = 1; i<gridSize-1; i++) {
			for (int j = 1; j<gridSize-1; j++) {
					spinenergy += grid[i][j]*(grid[i-1][j]+grid[i+1][j]+grid[i][j-1]+grid[i][j+1]);
					//spinenergy+=grid[i][j];
			}
		}
	//if(logfile.is_open()){logfile << spinenergy << endl;}
	totalEnergy = spinenergy/(1600.0*J);
	// double gridpoint = grid[gridSize-1][0];
...
double IsingSystem::correlfunc() {
	...
	double correl=(1/N)*(pow(green,2));

	//if(logfile.is_open()){logfile << Magnetisation << endl;}
	//if(numParticles %1 == 0 || numParticles % 100 == 0 && logfile.is_open()){logfile << Magnetisation << endl;}
	return correl;
}
...
// if (numParticles % 1 == 0 && logfile.is_open()) {
// 				logfile << lastP->pos[0] << " " <<  lastP->pos[1] << endl;	